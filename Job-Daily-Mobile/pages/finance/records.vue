			...
			<view class="body">
				<view class="item" v-for="(item, index) in list" :key="index">
					<view class="space-between item-top">
						...
						<text v-if="item.addSign == '+'" style="color: green;">
							+{{ item.money }}元
						</text>
						<text v-else-if="item.addSign == '-'" style="color: red;">
							-{{ item.money }}元
						</text>
						<text v-else style="color: grey;">
							{{ item.money }}元
						</text>
					</view>
...
export default {
	...
	onLoad(options) {
		if(options.tradeType){
			this.query.tradeType=options.tradeType;
			if(options.tradeType==1){
				uni.setNavigationBarTitle({
					title:'工资收入'
				})
			}else if(options.tradeType==2){
				uni.setNavigationBarTitle({
					title:'工资支出'
				})
			}

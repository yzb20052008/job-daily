<template>
	<view class="content">
		...
		<!-- <f-navbar bgColor="#007aff" :navbarType="5" fontColor="#333" transparentTitleColor="#000" :isShowLeft="true"
			:scrollTop="scrollTop" title=" " :isShowTransparentTitle="false">
			<view class="navbar-slot" slot="left">
				<view class="location" >
					<text class="location-name">小蓝零工</text> 
				</view>
						...
						<view>
							<view class="city-select" @click="selectCity">
								<text class="city-name"
									:class="cityOn?'city-name-on':'city-name-off'">{{cityInfo.area}}</text>
								<text class="city-icon yzb yzb-zhijiao-triangle"
									:class="cityOn?'city-icon-on':'city-icon-off'"></text>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			getLocation() {
				...
				loGetLocation(
					res => {
						...
						// 有 adcode 用新城市；否则用缓存/默认「附近」，保证列表可体验
			...
			upCallback(page) {
				...
				this.getPostList(page.num, page.size);
			},

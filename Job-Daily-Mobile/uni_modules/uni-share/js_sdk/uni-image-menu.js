...
class NvImageMenu {
	...
	show({
		...
		var margin = 20,
			...
			icontextSpace = 5,
			textHeight = 12
		var left1 = (screenWidth - (iconWidth * 2)) / 3
		var iconSpace = screenWidth - (left1 * 2) - (iconWidth * 2) //屏幕宽度减去左右留白间距，再减去4个图标的宽度，就是3个同行图标的间距
		if (iconSpace <= 5) { //屏幕过窄时，缩小边距和图标大小，再算一次
			margin = 15
			iconWidth = 40
			left1 = margin / 360 * screenWidth
			iconSpace = screenWidth - (left1 * 2) - (iconWidth * 4) //屏幕宽度减去左右留白间距，再减去4个图标的宽度，就是3个同行图标的间距
		}
		...
		list.forEach((item, i) => {
			...
			myList.push({
				...
				position: {
					top: margin / 2 + iconWidth + textHeight,
					left: LEFT['left' + (1 + i % 4)]-margin,
					width: iconWidth * 2,
					height: textHeight
				}
			})

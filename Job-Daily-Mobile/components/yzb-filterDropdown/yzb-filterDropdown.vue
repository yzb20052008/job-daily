	...
	<view class="HMfilterDropdown" :class="{'setDropdownBottom':maskVisibility}" :style="{'top':menuTop+'rpx'}" @touchmove.stop.prevent="discard" @tap.stop="discard">
		...
		<block v-for="(page,page_index) in subData" :key="page_index">
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				<block v-if="page.type=='hierarchy'&& page.child.length>0">
					<scroll-view class="sub-menu-list left" :class="[activeMenuArr[page_index].length>1?'first':'alone']"
					 :scroll-y="true" :scroll-into-view="'first_id'+firstScrollInto">
						<block v-for="(sub,index) in page.child" :key="index">
					...
					<block v-for="(sub,index) in page.child" :key="index">
						<scroll-view class="sub-menu-list not-first" :scroll-y="true" v-if="activeMenuArr[page_index][0]==index&&sub.child.length>0"
						 :scroll-into-view="'second_id'+secondScrollInto">
							<block v-for="(sub_second,second_index) in sub.child" :key="second_index">
								<view class="sub-menu" :id="'second_id'+second_index" :class="{'on':activeMenuArr[page_index][1]==second_index}">
									...
									<view class="more-sub-menu" v-if="sub_second.child&&sub.child.length>0&&sub_second.child.length>0">
										<block v-for="(sub2,sub2_index) in sub_second.child" :key="sub2_index">
											...
											<text v-if="sub_second.showAllSub!=true && sub2_index==8 && sub_second.child.length>9" @tap="showMoreSub(second_index)">更多<text
				...
				<block v-if="page.type=='filter'">
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.child" :key="box_index">
								<view class="title">{{box.name}}</view>
								<view class="labels">
									<view v-for="(label,label_index) in box.child" :key="label_index" @tap="selectFilterLabel(page_index,box_index,label_index)"
					...
					<view class="filter">
						<scroll-view class="menu-box" :scroll-y="true">
							<view class="box" v-for="(box,box_index) in page.child" :key="box_index">
								<view class="title">{{box.name}}</view>
								<view class="labels">
									<view v-for="(label,label_index) in box.child" :key="label_index" @tap="selectRadioLabel(page_index,box_index,label_index)"
									 :class="{'on':label.selected}">{{label.name}}</view>
								</view>
							</view>
						</scroll-view>
		...
		methods: {
				...
				for(var i=0;i<this.activeMenuArr.length;i++){
					...
					if(this.subData[i].type=='hierarchy'){
						if (typeof(row[0]) == 'number'){
							let tmpsub = this.subData[i].child[row[0]];
							if(row.length>1){
			...
			selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				...
				if (level3_index != null || level2_index != null || (level1_index != null && this.subData[page_index].child[level1_index].child.length == 0)
					...
					let sub = this.subData[page_index].child[level1_index].child[level2_index];
					if(this.updateMenuName){
			...
			resetFilterData(page_index) {
				...
				while (level > 0) {
					tmpArr.push([]);
					let box = this.subData[page_index].child[level - 1].child;
					...
					if (typeof(item[0]) == 'object') {
						...
						item.forEach((s, j) => {
							if(s!=null){
								...
								item[j] = s;
								s.forEach((v, k) => {
									value[i][j][k] = (v==null||v>=this.subData[i].child[j].child.length)?null:this.subData[i].child[j].child[v].id;
						...
						let child = this.subData[i].child[item[0]];
						value[i][0] = child.id;
			...
			processPage(index) {
				...
				if (this.menu[index].type == 'filter') {
					...
					for (let i = 0; i < level; i++) {
						let box = this.subData[index].child[i].child;
			...
			processActive(tmpitem) {
				...
				if (tmpitem.type == 'hierarchy'&&tmpitem.hasOwnProperty('child')&&tmpitem.child.length>0) {
					let level = this.getMaxFloor(tmpitem.child);
					while (level > 0) {
						tmpArr.push(null);
						level--;
					}
				} else if (tmpitem.type == 'filter') {
					let level = tmpitem.child.length;
					while (level > 0) {
						tmpArr.push([]);
						level--;
					}

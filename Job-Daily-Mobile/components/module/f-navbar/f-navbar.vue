                ...
                <view class="center" :style="{width:titleWidth+'rpx'}">
                    <slot name="default">
						<block v-if="navbarType == 5 && transparentTitleColor && isShowTransparentTitle">
							<!-- 透明背景时实现上拉title颜色改变 -->
							<view class="title u-line-1" :style="[{opacity: 1-transparentValue},{color:transparentTitleColor,width:titleWidth+'rpx',fontSize:fontSize+'rpx'}]" v-if="transparentValue<proportion">{{title}}</view>

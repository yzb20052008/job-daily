<template>
	<view class="uni-grid" :class="setBorderClass">
		<view class="uni-grid__flex" v-if="gridGroup.length>0" v-for="(items,i) in gridGroup" :key="i">
			<view class="uni-grid-item" hover-class="uni-grid-item-hover" :hover-start-time="20" :hover-stay-time="70"
				v-for="(item,index) in items" :key="index"
				:class="[index === columnNumber ? 'uni-grid-item-last' : '','uni-grid-item-' + type]"
				:style="{flexBasis:100/columnNumber + '%'}" @click="onClick(item)">
				<button open-type="contact" class="uni-grid-item__content" v-if="item.openUrl=='contact'"
					@contact="onContact">
					<view style="line-height: 1;">
						<image class="uni-grid-item-image" :src="item.icon"></image>
					</view>
					<text class="uni-grid-item-text" :class="role=='member'?'mt-10':''">{{item.name}}</text>
				</button>
				<view class="uni-grid-item__content" v-else>
					<view style="line-height: 1;">
						<image class="uni-grid-item-image" :src="item.icon"></image>
					</view>
					<text class="uni-grid-item-text">{{item.name}}</text>
				</view>
			</view>
		</view>
	</view>

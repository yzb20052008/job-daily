		...
		<view class="post-item" v-for="(item,index) in list" :key="index" @click="itemClick(item)">
			<view class="worker">
				<view class="worker-left">
					<u-avatar :src="item.avatar" size="50" shape="square"></u-avatar>
			...
			<text class="introduction">{{item.persionSkill}}</text>
			<view class="bottom">
				<view class="location">
	...
	export default {
		...
		methods: {
			...
			itemClick(item) {
				...
				uni.$u.route("/pages/resume/resumeDetail?id=" + item.dataId);
			},
			// 取消收藏
			cancel(item) {

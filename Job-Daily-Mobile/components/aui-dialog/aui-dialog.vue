<template name="aui-dialog">
	<view class="aui-dialog" v-if="SHOW" :class="{'aui-dialog-in': FADE==1, 'aui-dialog-out': FADE==0}">
		...
		>
			<view class="aui-dialog-title" v-if="title" v-html="title"></view>
			...
			<view class="aui-dialog-down">
				<view v-if="isButton==false"
					...
					open-type="openSetting" @opensetting="opensetting"
				>{{item.name}}</view>
				<button v-if="isButton==true"
					class="aui-dialog-down-btn"
					v-for="(item, index) in btns" 
					:class="{'aui-dialog-down-cancel-btn': item.name=='取消', 'aui-dialog-down-delete-btn': item.name=='删除'}"
					:key="index" 
					...
					@touchmove="_btnTouchEnd($event)"
					@touchend="_btnTouchEnd($event)"
					:open-type="index==1?'openSetting':''" @opensetting="opensetting"
				>{{item.name}}</button>
				<!-- <button class="btn" type="primary" size="mini" open-type="openSetting" @opensetting="opensetting">设置</button> -->
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods:{
			...
			opensetting(data){
				this.$emit("opensetting",data.target.authSetting);
			},

...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	@Override
	public LoginUser getUserByThirdId(String thirdId) {
		if (oConvertUtils.isEmpty(thirdId)) {
			return null;
		}
		LoginUser loginUser = new LoginUser();
		SysUser sysUser = sysUserService.getUserByThirdId(thirdId);
		if(sysUser==null) {
			return null;
		}
		BeanUtils.copyProperties(sysUser, loginUser);
...
//		}
	...
	@Override
	public LoginUser getUserByTtId(String ttId) {
		if (oConvertUtils.isEmpty(ttId)) {
			return null;
		}
		LoginUser loginUser = new LoginUser();
		SysUser sysUser = sysUserService.getUserByTtId(ttId);
		if(sysUser==null) {
			return null;
		}
		BeanUtils.copyProperties(sysUser, loginUser);
	...
	@Override
	public LoginUser getUserByInviteCode(String inviteCode) {
		if (oConvertUtils.isEmpty(inviteCode)) {
			return null;
		}
		LoginUser loginUser = new LoginUser();
		SysUser sysUser = sysUserService.getUserByInviteCode(inviteCode);
		if(sysUser==null) {
			return null;
		}
		BeanUtils.copyProperties(sysUser, loginUser);
...
//		}
	...
	@Override
	public List<Map<String, String>> getRoleMapByUserName(String username) {
		return sysUserRoleMapper.getRoleMapByUserName(username);
	}

	@Override
	public List<Map<String, String>> getRoleByPlatform(String platform) {
		return sysUserRoleMapper.getRoleByPlatform(platform);
	}
...
//			}
	...
	@Override
	public Result<?> getUserPermissionByToken(String username) {
//		Result<JSONObject> result = new Result<JSONObject>();
		try {
			...
			List<SysPermission> metaList2 = sysPermissionMapper.queryByUser(loginUser.getUsername());
			List<SysPermission> metaList=new ArrayList<>();
			//去除online
			for (SysPermission permission:metaList2){
				//移动端去除多余表单20230815
				if ("在线开发".equals(permission.getName()) || (permission.getParentId()!=null && "e41b69c57a941a3bbcce45032fe57605".equals(permission.getParentId()))){
				}else{
					metaList.add(permission);
				}
			}
			JSONObject json = new JSONObject();
			...
			query.eq(SysPermission::getPlatform,loginUser.getPlatform());//新增平台判断
			//query.eq(SysPermission::getStatus, "1");
			List<SysPermission> allAuthList = sysPermissionMapper.selectList(query);
...
//			result.error500("查询失败:" + e.getMessage());
	...
	@Override
	public List<SelectTreeModel> loadCategoryList(String pcode) {
		return sysCategoryService.loadTreeRoot(pcode);
	}
	...
	@Override
	public void updateUserInfo(LoginUser loginUser, String selectedRoles, String selectedDeparts, String relTenantIds) {
		SysUser sysUser = new SysUser();
		BeanUtils.copyProperties(loginUser, sysUser);
		sysUserService.editUser(sysUser,selectedRoles,selectedDeparts,relTenantIds);
	}
	...
	@Override
	public void saveUser(LoginUser loginUser, String selectedRoles, String selectedDeparts, String relTenantIds) {
		SysUser sysUser = new SysUser();
		BeanUtils.copyProperties(loginUser, sysUser);
		sysUserService.saveUser(sysUser,selectedRoles,selectedDeparts,relTenantIds);
	}

	@Override
	public Result<?> checkUserIsEffective(LoginUser loginUser) {
		SysUser sysUser = new SysUser();
		if (loginUser != null){
			BeanUtils.copyProperties(loginUser, sysUser);
		}else{
	...
	@Override
	public void changePassword(String userId, String username, String password) {
		SysUser sysUser=new SysUser();

...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	@Override
	public List<SelectTreeModel> loadTreeRoot(String pcode) {
		List<SelectTreeModel> ls = toList(this.queryListByCode(pcode));
		loadAllCategoryChildren(ls);
		return ls;
	}

	private List<SelectTreeModel> toList(List<TreeSelectModel> ls){
		List<SelectTreeModel> list=new ArrayList<>();
		SelectTreeModel treeSelectModel;
		for (TreeSelectModel model:ls){
			treeSelectModel=new SelectTreeModel();
			BeanUtils.copyProperties(model, treeSelectModel);
			list.add(treeSelectModel);
		}
		return list;
	}
	 ...
	 * 递归求子节点 同步加载用到
	 */
	private void loadAllCategoryChildren(List<SelectTreeModel> ls) {
		for (SelectTreeModel tsm : ls) {
			List<SelectTreeModel> temp = toList(this.queryListByPid(tsm.getKey()));

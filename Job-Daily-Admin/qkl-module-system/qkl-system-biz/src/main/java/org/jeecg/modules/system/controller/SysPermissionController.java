...
public class SysPermissionController {
	...
	public Result<List<SysPermissionTree>> list(SysPermission sysPermission, HttpServletRequest req) {
		...
		try {
			...
			if(oConvertUtils.isNotEmpty(sysPermission.getPlatform())){
				query.eq(SysPermission::getPlatform,sysPermission.getPlatform());
			}
			//支持通过菜单名字，模糊查询
...
//	}
	...
	public Result<Map<String, Object>> queryTreeList(@RequestParam(name = "platform", defaultValue = "admin", required = false) String platform) {
		...
		try {
			...
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.orderByAsc(SysPermission::getSortNo);
			if(oConvertUtils.isNotEmpty(platform)){
				query.eq(SysPermission::getPlatform,platform);
			}
			List<SysPermission> list = sysPermissionService.list(query);

...
public class SysUserController {
	...
	public Result<IPage<SysUser>> queryPageList(SysUser user,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        if (oConvertUtils.isNotEmpty(user.getUserIdentity())){
            userIndentity=user.getUserIdentity();
            user.setUserIdentity(null);
        }
		QueryWrapper<SysUser> queryWrapper = QueryGenerator.initQueryWrapper(user, req.getParameterMap());
        if (userIndentity ==3){
            queryWrapper.eq("user_identity",3);
        }else if(userIndentity>0){
            queryWrapper.ne("user_identity",3);
        }
        String keyword = req.getParameter("keyword");
        if(oConvertUtils.isNotEmpty(keyword)){
            queryWrapper.apply("( nickname like '%"+keyword+"%'  or phone like '%"+keyword+"%')");
        }

        //------------------------------------------------------------------------------------------------
        //是否开启系统管理模块的多租户数据隔离【SAAS多租户模式】
        String tenantId = oConvertUtils.getString(TenantContext.getTenant(), "0");
        if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL && !tenantId.equals("0")) {
//            String tenantId = oConvertUtils.getString(TenantContext.getTenant(), "0");
    ...
    public Result<IPage<Map<String,Object>>> list2(SysUser user, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                                   @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize, HttpServletRequest req) {
        Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);
        IPage<Map<String,Object>> pageList=sysUserService.getUserList(page, user);

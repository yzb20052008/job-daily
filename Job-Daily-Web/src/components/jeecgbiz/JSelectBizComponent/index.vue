<template>
  <a-row class="j-select-biz-component-box" type="flex" :gutter="8">
    ...
    <a-col v-if="buttons" class="right">
      <a-button type="primary" icon="search" :disabled="disabled" @click="showModel">{{selectButtonText}}</a-button>
    </a-col>
  ...
  export default {
    ...
    watch: {
      value: {
        ...
        handler(val) {
          ...
          let arr=this.selectedValue.concat(this.selectValue);
          //去重
          let newArr=[];
          ...
          this.selectedValue=newArr;
          console.log("==selectedValue==",newArr);
        }
      },
      selectedValue: {
        deep: true,
    ...
    methods: {
      ...
      handleOptions(options, dataSourceMap) {
        console.log("==handleOptions==",options,dataSourceMap);
